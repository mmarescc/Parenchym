...
class Rc:
    ...
    .. warning::
        ...
        mention them in your ``.gitignore`` file. So they won't be published
        e.g. on github.
        Sample ``.gitignore``::
    ...
    def __init__(self, environment, root_dir=None, etc_dir=None):
        ...
        if root_dir is None:
            # assume, INI file is in root_dir
            root_dir = os.getcwd()
        if etc_dir is None:
        ...
        self.debug = 0
        self._re_ref_node = re.compile(
            r'(?P<prefix>.+\.)rc_ref_(?P<infix>.+)', re.U)
        self._ext = '.yaml'
    ...
    def load(self, fn=None, key=None):
        ...
        :param fn: Name of rc file without path, but including extension.
    ...
    def load_file(self, fn, key, allow_empty=False):
        ...
        :param fn: Full name of rc file (path, name and extension).
        :param key: Loaded data is stored with this key
        ...
        if self.debug:
            print("Loading rc into key '{0}' from file '{1}'".format(key, fn))
        ...
        if self.debug:
            print("Loading rc secrets into key '{0}' from file '{1}'".format(
                key, fn))
        try:
            with open(fn, 'rb') as stream:
                dd = load(stream)
    ...
    def resolve_references(self):
        ...
        for k, v in self.data.items():
            ...
            for ref_k, ref_v in self.data.items():
                if not ref_k.startswith(ref_prefix):
                    continue
